	...
	private function addHeadDeclarations($id, $data = null) {
		...
		if (empty($data)) {
			...
			foreach ($data as $imagedata) {
				...
				if ($imagedata->hyperlink) {
					$array[$imagedata->imageurl]['target'] = $this->params->target;
				}
			}
		}
		$options = array(
		...
		$document->addScriptDeclaration('window.addEvent("load", function () { new '.$class.'(document.id("'.$id.'").removeClass("showplus-noscript").getElement("div"), '.json_encode($array).', $extend('.json_encode($options).', {'.implode(',',$args).'})); });');
	}
