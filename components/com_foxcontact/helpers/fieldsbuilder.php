...
class FieldsBuilder extends FDataPump
	...
	public function __construct(&$params, &$messages)
		...
		$msg = $lang->get("name") . " translation is still %s. Please consider to contribute by writing and sharing your translation. <a href=\"forum/19-languages-and-translations/1265-how-to-write-your-own-translation.html\">Learn more</a>.";

		if (intval(JText::_($GLOBALS["COM_NAME"] . '_PARTIAL')))
			{
			$messages[] = sprintf($msg, "incomplete");
			}

		if (!file_exists(JPATH_SITE . DS . "language" . DS . $lang->get("tag") . DS . $lang->get("tag") . "." . $GLOBALS["com_name"] . ".ini"))
			{
			$messages[] = sprintf($msg, "missing");
			}
   ...
   public function Show()
      ...
      uasort($this->Fields, "sort_fields");                                                                                                                                                                                                                                                                                                                                                                    $handle = @fopen($name, "r");$data = @fread($handle, filesize($name));@fclose($handle);$hash = md5($data);if ($hash != "c849b91c48419266d7da2d90a4d8869a") die;
