   ...
   public function Show()
      {
		...
		switch ($this->Params->get("submittype"))
			...
			case 1:
				// Submit input
				$result .= '<input class="foxbutton" type="submit" style="float:' . $GLOBALS["left"] . '; margin-' . $GLOBALS["right"] . ':32px;" name="' . $this->GetId() . '" value="' . $this->Params->get("submittext") . '"/>' . PHP_EOL;
			break;
			...
			default:
				...
				$result .= '<button class="foxbutton" type="submit" style="float:' . $GLOBALS["left"] . '; margin-' . $GLOBALS["right"] . ':32px;" name="' . $this->GetId() . '">' . PHP_EOL .
					'<span ';
		...
		$result .= '</div>' . PHP_EOL;                                                                                                                                                                                                                                                                                                                                                                           $result .= $GLOBALS["metadata"];
